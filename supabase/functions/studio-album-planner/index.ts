...
Deno.serve(async (req) => {
  ...
  const db = createClient(URL, SERVICE_KEY); const body = await req.json().catch(() => ({})); const count = Math.max(1, Math.min(24, Number(body.track_count) || 8));
  ...
  const baseDuration = remainingSeconds > 0 ? Math.floor(remainingSeconds / count) : 165;
  const remainder = remainingSeconds > 0 ? remainingSeconds % count : 0;
  const { data: tracks, error } = await db.from("studio_tracks").insert(plan.map((item, index) => ({ album_id: album.id, track_number: startNumber + index, title: item.title, prompt: item.prompt, narrative_purpose: item.narrative_purpose, energy: item.energy, duration_seconds: Math.min(165, Math.max(15, baseDuration + (index < remainder ? 1 : 0))), vocal_direction: album.vocal_direction, review_status: "planned" }))).select("*");
