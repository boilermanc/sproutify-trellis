...
const MAX_TRACK_SECONDS = 165;
const MAX_TRACKS = 40;
const CORS = { "Access-Control-Allow-Origin": "*", "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type", "Access-Control-Allow-Methods": "POST, OPTIONS" };
const json = (body: unknown, status = 200) => new Response(JSON.stringify(body), { status, headers: { ...CORS, "content-type": "application/json" } });

function fallback(album: any, count: number, startNumber: number) {
  return Array.from({ length: count }, (_, index) => ({
    title: `${album.title} — ${startNumber + index === 1 ? "Opening Signal" : `Movement ${startNumber + index}`}`,
...
function distributeRuntime(totalSeconds: number, count: number): number[] {
  if (totalSeconds < count * MIN_TRACK_SECONDS || totalSeconds > count * MAX_TRACK_SECONDS) {
    throw new Error(`${count} tracks cannot cover the remaining ${totalSeconds} seconds. Choose between ${Math.ceil(totalSeconds / MAX_TRACK_SECONDS)} and ${Math.floor(totalSeconds / MIN_TRACK_SECONDS)} tracks.`);
  }
...
Deno.serve(async (req) => {
  ...
  const db = createClient(URL, SERVICE_KEY); const body = await req.json().catch(() => ({})); const count = Math.max(1, Math.min(MAX_TRACKS, Math.round(Number(body.track_count) || 8)));
  ...
  const { data: existingTracks, error: existingError } = await db.from("studio_tracks").select("track_number, title, duration_seconds").eq("album_id", album.id).order("track_number");
  if (existingError) return json({ error: existingError.message }, 500);
  const existing = existingTracks || [];
  const startNumber = existing.reduce((maximum, track) => Math.max(maximum, Number(track.track_number) || 0), 0) + 1;
  const existingSeconds = existing.reduce((total, track) => total + (Number(track.duration_seconds) || 0), 0);
  const remainingSeconds = Math.max(0, Number(album.target_duration_seconds) - existingSeconds);
  let durations: number[];
  try {
    durations = remainingSeconds > 0 ? distributeRuntime(remainingSeconds, count) : Array.from({ length: count }, () => 150);
  } catch (error) {
    return json({ error: error instanceof Error ? error.message : "The requested track count cannot cover the remaining runtime." }, 400);
  }
  const fallbackPlan = fallback(album, count, startNumber);
  ...
  if (key) try {
    const priorTitles = existing.slice(-12).map(track => `${track.track_number}. ${track.title}`).join("; ");
    const style = album.style_profile || {};
    const prompt = `Create exactly ${count} original additional tracks that continue one cohesive AI music album. Return only a JSON array of objects: {"title":"...","prompt":"...","narrative_purpose":"...","energy":1-10}. Album: ${album.title}; fictional artist: ${album.artist_name}; genre: ${album.genre || "open"}; mood: ${album.mood || "open"}; era: ${album.era || "open"}; setting: ${album.theme || "open"}; vocals: ${album.vocal_direction}; instruments: ${Array.isArray(style.instruments) ? style.instruments.join(", ") : "genre appropriate"}; tempo range: ${style.bpm_range || "varied but cohesive"}; house direction: ${style.prompt_guidance || "none"}; next track number: ${startNumber}; existing tracklist: ${priorTitles || "none"}. Avoid duplicate titles and create a natural continuation with a deliberate opening, development, peak, cooldown, and resolution across the full record. Each prompt is one concise 15–30 word sentence describing instruments, style, mood, and BPM. No real artists, songs, brands, franchises, lyrics, or unsafe themes.`;
  ...
  const { data: tracks, error } = await db.from("studio_tracks").insert(plan.map((item, index) => ({ album_id: album.id, track_number: startNumber + index, title: item.title, prompt: item.prompt, narrative_purpose: item.narrative_purpose, energy: item.energy, duration_seconds: durations[index], vocal_direction: album.vocal_direction, review_status: "planned" }))).select("*");
  if (error) return json({ error: error.message }, 500);
  await db.from("studio_albums").update({ status: "track_planning", music_generation_status: "not_started", updated_at: new Date().toISOString() }).eq("id", album.id);
  return json({ tracks });
});
