...
- Composition registry: render jobs accept only pinned key/version pairs. `PromoProof@ps-002-v1` is Rekkrd-only, while `vertical-ui-story@v1` is the branch-neutral contract reserved for all branches; both remain disabled in the production worker until the generic Remotion implementation exists.
