...
5. The no-op worker is intentionally the only executable worker type. Capture, voice, alignment, music, rendering, export, and publishing remain queued architecture until their corresponding workers are implemented. Render activation is now blocked only by the isolated executor and deliberate composition enablement; presentation and generic FFmpeg fingerprint gates are satisfied for Rekkrd.
