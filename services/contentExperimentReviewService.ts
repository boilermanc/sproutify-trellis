import type { ContentExperiment, ContentPost } from './contentIntelligenceRegistry';
...
export function getExperimentReviewState(
  ...
  const evaluationDays = Number.isFinite(experiment.evaluation_window_days)
    ? Math.max(0, experiment.evaluation_window_days)
    : 0;
  const due = new Date(baselineDate.getTime() + evaluationDays * DAY_MS);
