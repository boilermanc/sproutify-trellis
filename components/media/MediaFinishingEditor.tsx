...
import type { Branch, MediaGenerationLibraryItem, MediaTextCue, MediaTextStyle } from '../../types';
import { createMediaFinishingJob } from '../../services/mediaGenerationService';
import TimedTextTimeline from './TimedTextTimeline';
import VideoResultPreview from './VideoResultPreview';
import { MEDIA_FONT_FAMILIES, MEDIA_FONT_OPTIONS, mediaFontIdForFamily } from './mediaFonts';

interface Props {
  item: MediaGenerationLibraryItem;
  branches: Branch[];
  onClose: () => void;
...
const MediaFinishingEditor: React.FC<Props> = ({ item, branches, onClose, onQueued, addToast }) => {
  const branch = branches.find(candidate => candidate.id === item.project?.branch_id);
  const duration = Math.max(1, Number(item.asset.duration_seconds || 1));
  const initialCues = Array.isArray(item.job.parameters?.finishing && (item.job.parameters.finishing as Record<string, unknown>).text_cues)
    ? ((item.job.parameters.finishing as Record<string, unknown>).text_cues as MediaTextCue[])
    : [];
  const [cues, setCues] = useState<MediaTextCue[]>(initialCues);
  const [style, setStyle] = useState<MediaTextStyle>({
  ...
  const submit = async () => {
    ...
    try {
      setSubmitting(true);
      await createMediaFinishingJob({
        source_output_id: item.output_id,
        text_cues: cues,
        style,
        idempotency_key: crypto.randomUUID(),
      });
      addToast('Final video queued. The original is unchanged.', 'success');
      await onQueued();
      onClose();
    } catch (error) {
      addToast(error instanceof Error ? error.message : 'Could not queue the final video.', 'error');
    } finally {
      setSubmitting(false);
    }
  };

  return <div className="fixed inset-0 z-[70] overflow-y-auto bg-slate-950/75 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Finish generated video">
    <div className="mx-auto my-4 w-full max-w-6xl overflow-hidden rounded-[2rem] bg-white shadow-2xl">
      <div className="flex items-center justify-between border-b border-slate-200 px-6 py-5">
        <div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-violet-600">Post-generation finishing</p><h2 className="mt-1 text-xl font-black text-slate-900">Add text to the finished video</h2><p className="mt-1 text-xs text-slate-500">The original AI video stays untouched. Rendering this version does not start a GPU generation.</p></div>
        <button type="button" onClick={onClose} aria-label="Close finishing editor" className="rounded-xl border border-slate-200 p-2 text-slate-500 hover:bg-slate-50"><X className="h-5 w-5" /></button>
      </div>

      <div className="grid gap-6 p-6 lg:grid-cols-[1.15fr_.85fr]">
        <div className="space-y-5">
          {item.signed_url && <VideoResultPreview src={item.signed_url} cues={cues} style={style} />}
          <TimedTextTimeline durationSeconds={duration} cues={cues} onChange={setCues} showPreview={false} />
        </div>

        <aside className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 lg:sticky lg:top-4 lg:self-start">
          <div><div className="flex items-center gap-2 text-sm font-black text-slate-900"><Type className="h-4 w-4 text-violet-600" /> Typography</div><p className="mt-1 text-xs leading-5 text-slate-500">Choose any available font now. Brand-specific font collections can plug into this picker later.</p></div>
          ...
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-xs leading-5 text-emerald-800"><strong>No LongCat charge:</strong> this is a finishing render using the existing Trellis video worker.</div>
          <div className="flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-black text-slate-600">Cancel</button><button type="button" disabled={!canRender || submitting} onClick={() => void submit()} className="flex items-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-xs font-black text-white disabled:opacity-40">{submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <WandSparkles className="h-4 w-4" />} Render final video</button></div>
        </aside>
      </div>
    </div>
  </div>;
...
export default MediaFinishingEditor;
