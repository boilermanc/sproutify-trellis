...
const LeadSequencePanel: React.FC<Props> = ({ enrollment, loading, disabled, previewFirstName, previewRecipientEmail, previewScope, onStart, onAction }) => {
  ...
  if (loading) return <section className="rounded-sm border border-white/10 bg-white/[0.025] p-4"><Loader2 className="animate-spin text-emerald-300" size={18} /></section>;

  if (!enrollment) return (
    <section className="rounded-sm border border-emerald-400/20 bg-emerald-400/[0.04] p-4">
      <div className="mb-3 flex items-center gap-2"><MailCheck className="text-emerald-300" size={17} /><h3 className="text-[10px] font-black   text-emerald-200">Farm follow-up sequence</h3></div>
      ...
      <div className="mt-3 flex gap-2">
        <button type="button" onClick={() => setMode('approval')} className={`rounded-sm border px-3 py-2 text-[10px] font-black uppercase tracking-wider ${mode === 'approval' ? 'border-emerald-400/40 bg-emerald-400/10 text-emerald-200' : 'border-white/10 text-slate-500'}`}>Approve each</button>
        <button type="button" onClick={() => setMode('automatic')} className={`rounded-sm border px-3 py-2 text-[10px] font-black uppercase tracking-wider ${mode === 'automatic' ? 'border-emerald-400/40 bg-emerald-400/10 text-emerald-200' : 'border-white/10 text-slate-500'}`}>Automatic</button>
      </div>
      <label className="mt-3 flex cursor-pointer items-start gap-2 rounded-sm border border-white/15 bg-slate-950/35 px-3 py-2.5 text-[10px] font-bold leading-4 text-white">
      ...
      <button type="button" disabled={working || disabled || !referralConfirmed} onClick={() => void run(() => onStart(mode))} className="mt-3 inline-flex items-center gap-2 rounded-sm bg-emerald-400 px-4 py-2.5 text-[10px] font-black uppercase tracking-wider text-[#0a2420] disabled:cursor-not-allowed disabled:opacity-40">{working ? <Loader2 size={14} className="animate-spin" /> : <Play size={14} />}Create sequence & choose email</button>
    </section>
  );
  ...
  return (
    <>
    <section className="rounded-sm border border-emerald-400/20 bg-emerald-400/[0.035] p-4">
      <div className="flex items-start justify-between gap-3">
        <div><h3 className="text-[10px] font-black   text-emerald-200">{enrollment.sequence?.name || 'Lead sequence'}</h3><p className={`mt-1 text-[10px] font-bold ${exitDetails ? 'text-rose-300' : 'capitalize text-slate-500'}`}>{enrollment.mode} · {exitDetails?.label || enrollment.status.replace(/_/g, ' ')}</p></div>
        {working && <Loader2 size={16} className="animate-spin text-emerald-300" />}
      </div>
      {enrollment.status === 'awaiting_approval' && <p className="mt-3 rounded-sm border border-emerald-300/20 bg-emerald-300/[0.07] px-3 py-2 text-[10px] font-bold text-emerald-100">Click anywhere on an unsent email card to select it. Click its number only to preview.</p>}
      <div className="mt-4 space-y-2">
        {enrollment.steps.map(step => {
          ...
          return <div key={step.id} onClick={() => { if (selectable && !working) setSelectedStepId(step.id); }} className={`flex w-full items-center gap-3 rounded-sm border px-3 py-2.5 text-left transition ${selected ? 'border-emerald-400 bg-emerald-50 ring-2 ring-emerald-100' : 'border-slate-200 bg-white'} ${selectable ? 'cursor-pointer hover:border-emerald-300 hover:bg-emerald-50/60' : ''}`}>
            <button type="button" onClick={event => { event.stopPropagation(); setPreviewStep(step); }} aria-label={`Preview email ${step.step_number}: ${step.name}`} title={`Preview email ${step.step_number}`} className={`group relative flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[9px] font-black transition hover:scale-105 hover:ring-2 hover:ring-emerald-200 ${message?.sent_at ? 'bg-emerald-100 text-emerald-700' : isNext ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
              {step.step_number}
              <Eye size={9} className="absolute -bottom-1 -right-1 rounded-full bg-white text-emerald-600 opacity-0 transition group-hover:opacity-100" />
            </button>
      ...
      {exitDetails && <div className="mt-3 rounded-sm border-2 border-rose-400 bg-rose-950 px-4 py-3 text-white" role="alert">
      ...
      {active && <div className="mt-3 flex flex-wrap gap-2">
        {enrollment.status === 'awaiting_approval' && <button type="button" disabled={working || !approvalArmed} onClick={() => selectedApprovalStep && void run(() => onAction('approve_next', selectedApprovalStep.step_number))} className="inline-flex items-center gap-1.5 rounded-sm bg-emerald-400 px-3 py-2 text-[9px] font-black uppercase tracking-wider text-[#071b25] disabled:cursor-not-allowed disabled:opacity-40"><Send size={12} />Approve & send{selectedApprovalStep ? ` email ${selectedApprovalStep.step_number}` : ''}</button>}
        {enrollment.status === 'active' && <button type="button" disabled={working} onClick={() => void run(() => onAction('pause'))} className="inline-flex items-center gap-1.5 rounded-sm border border-white/10 px-3 py-2 text-[9px] font-black uppercase tracking-wider text-slate-300"><Pause size={12} />Pause</button>}
        {enrollment.status === 'paused' && <button type="button" disabled={working} onClick={() => void run(() => onAction('resume'))} className="inline-flex items-center gap-1.5 rounded-sm border border-emerald-400/30 px-3 py-2 text-[9px] font-black uppercase tracking-wider text-emerald-300"><Clock3 size={12} />Resume</button>}
        <button type="button" disabled={working} onClick={() => void run(() => onAction('stop'))} className="inline-flex items-center gap-1.5 rounded-sm border border-rose-400/20 px-3 py-2 text-[9px] font-black uppercase tracking-wider text-rose-300"><StopCircle size={12} />Stop</button>
      </div>}
    </section>
    {previewStep && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/80 p-3 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={`Email ${previewStep.step_number} preview`} onMouseDown={event => { if (event.target === event.currentTarget) setPreviewStep(null); }}>
      <div className="flex h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-sm border border-white/10 bg-[#10142E] ">
        <div className="flex items-start justify-between gap-4 border-b border-white/10 px-5 py-4">
          <div className="min-w-0">
            <p className="text-[10px] font-black uppercase tracking-widest text-emerald-300">Email {previewStep.step_number} · {previewStep.name}</p>
          ...
          <button type="button" onClick={() => setPreviewStep(null)} className="rounded-sm border border-white/10 p-2 text-slate-400 transition hover:bg-white/10 hover:text-white" aria-label="Close email preview"><X size={18} /></button>
        </div>
        <div className="min-h-0 flex-1 bg-slate-200 p-3 sm:p-5">
          <iframe title={`Email ${previewStep.step_number}: ${previewStep.name}`} srcDoc={previewHtml} sandbox="allow-popups" className="h-full w-full rounded-sm border-0 bg-white " />
        </div>
      </div>
    </div>}
    </>
