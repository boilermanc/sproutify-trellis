...
const LeadActivityModal: React.FC<LeadActivityModalProps> = ({ lead, kind, pending, onClose, onSubmit }) => {
  ...
  return (
    <CrmModal title={config.title} subtitle={lead.profile?.email || 'Lead activity'} icon={config.icon} pending={pending} onClose={onClose}>
      ...
      >
        {kind === 'call' && <label className="block space-y-2 text-xs font-bold text-slate-300">Outcome<select autoFocus value={outcome} onChange={event => setOutcome(event.target.value)} className="w-full rounded-sm border border-white/10 bg-[#0A0E27] p-3 text-white outline-none focus:border-emerald-400/50"><option value="connected">Connected</option><option value="voicemail">Voicemail</option><option value="no_answer">No answer</option></select></label>}
        {kind === 'meeting' && <label className="block space-y-2 text-xs font-bold text-slate-300">Meeting date *<input autoFocus type="date" value={meetingDate} onChange={event => setMeetingDate(event.target.value)} className="w-full rounded-sm border border-white/10 bg-[#0A0E27] p-3 text-white outline-none focus:border-emerald-400/50" /></label>}
        <label className="block space-y-2 text-xs font-bold text-slate-300">{kind === 'note' ? 'Note *' : 'Summary *'}<textarea autoFocus={kind === 'note'} rows={5} value={summary} onChange={event => setSummary(event.target.value)} className="w-full resize-y rounded-sm border border-white/10 bg-[#0A0E27] p-3 text-white outline-none placeholder:text-slate-600 focus:border-emerald-400/50" placeholder={kind === 'note' ? 'Add an internal note…' : 'What happened?'} /></label>
        {kind !== 'note' && <label className="block space-y-2 text-xs font-bold text-slate-300">Next follow-up<input type="date" value={nextFollowUp} onChange={event => setNextFollowUp(event.target.value)} className="w-full rounded-sm border border-white/10 bg-[#0A0E27] p-3 text-white outline-none focus:border-emerald-400/50" /></label>}
        <div className="flex justify-end gap-3 border-t border-white/10 pt-5">
          <button type="button" onClick={onClose} disabled={pending} className="rounded-sm px-4 py-3 text-xs font-black uppercase tracking-wider text-slate-400 hover:bg-white/5 disabled:opacity-40">Cancel</button>
          <button type="submit" disabled={pending || !valid} className="flex items-center gap-2 rounded-sm bg-[#059669] px-5 py-3 text-xs font-black uppercase tracking-wider text-white disabled:cursor-not-allowed disabled:opacity-40">{pending ? <Loader2 className="animate-spin" size={16} /> : React.createElement(config.icon, { size: 16 })} Save activity</button>
        </div>
      </form>
    </CrmModal>
  );
