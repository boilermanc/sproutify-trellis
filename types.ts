...
export type ViewState = 'dashboard' | 'profiles' | 'leads' | 'segments' | 'intelligence' | 'branches' | 'automations' | 'tasks' | 'email-preview' | 'dev-tools' | 'campaign-builder' | 'campaigns' | 'social-hub' | 'content-intelligence' | 'brand-intelligence' | 'settings' | 'support-hub' | 'reports' | 'knowledge-base' | 'help-center' | 'team' | 'user-profile' | 'platform-wizard' | 'marketing-wizard' | 'marketing-brands' | 'reddit-growth' | 'video-ad-lab' | 'media-generation' | 'motion-posts' | 'trellis-studio' | 'studio-albums' | 'trellis-episodes' | 'clip-studio' | 'ad-performance' | 'post-scheduler' | 'card-studio' | 'post-performance';
...
export interface StudioCoverConcept { id: string; version: number; image_url?: string | null; metadata_json?: { selection_status?: 'unselected' | 'selected' | 'approved'; direction?: string; role?: string; source_asset_id?: string; approved_cover_asset_id?: string; aspect_ratio?: string; enhancement_direction?: string; typography?: { title?: string; subtitle?: string; series?: string; treatment?: string; vintage_border?: boolean; title_color?: string; title_font?: string; subtitle_color?: string; subtitle_font?: string; series_color?: string; series_font?: string; text_v?: string; text_h?: string } }; }
