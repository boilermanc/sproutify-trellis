...
const renderDeepDiveBadge = (status?: LeadResearchStatus) => {
  ...
  if (status === 'running' || status === 'queued') {
    return <span title="Researching this lead…" className="inline-flex items-center gap-1 rounded-full border border-emerald-400/30 bg-emerald-400/10 px-2 py-0.5 text-[9px] font-black uppercase tracking-wider text-emerald-300"><Loader2 size={10} className="animate-spin" />Researching</span>;
  }
  if (status === 'failed') {
...
const Leads: React.FC<LeadsProps> = ({ branchContext, addToast }) => {
  ...
  return (
    <div className="leads-light space-y-6 rounded-sm bg-slate-50 p-5 text-slate-900   lg:p-8">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex items-center gap-4">
          <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-sm bg-emerald-400/10 text-emerald-300 ring-1 ring-emerald-400/20">
            <Target size={27} />
          </div>
          <div>
            ...
            {multipleBranchesSelected && activeBranch && (
              <p className="mt-1 text-[11px] text-emerald-300/70">Single-branch view · showing {activeBranch.name}, the first selected branch.</p>
            )}
          </div>
        </div>

        <div className="flex flex-wrap items-center gap-3">
          {selectedPipeline && <div className="flex rounded-sm border border-white/10 bg-[#10142E] p-1"><button type="button" onClick={() => setViewMode('list')} className={`flex items-center gap-1.5 rounded-sm px-3 py-2 text-[10px] font-black uppercase tracking-wider ${viewMode === 'list' ? 'bg-emerald-400 text-white' : 'text-slate-500 hover:text-white'}`}><List size={14} />List</button><button type="button" onClick={() => setViewMode('board')} className={`flex items-center gap-1.5 rounded-sm px-3 py-2 text-[10px] font-black uppercase tracking-wider ${viewMode === 'board' ? 'bg-emerald-400 text-white' : 'text-slate-500 hover:text-white'}`}><Columns3 size={14} />Board</button></div>}
          <button type="button" onClick={() => setShowEmailOutbox(true)} disabled={!selectedPipeline} className="flex items-center gap-2 rounded-sm border border-emerald-400/20 bg-emerald-400/[0.06] px-4 py-3 text-xs font-black uppercase tracking-wider text-emerald-200 transition hover:bg-emerald-400/10 disabled:cursor-not-allowed disabled:opacity-40"><MailCheck size={16} />Email Outbox</button>
          <button type="button" onClick={exportFilteredLeads} disabled={!selectedPipeline || filteredLeads.length === 0} className="flex items-center gap-2 rounded-sm border border-white/10 bg-white/5 px-4 py-3 text-xs font-black uppercase tracking-wider text-slate-200 transition hover:border-emerald-400/30 hover:bg-emerald-400/10 disabled:cursor-not-allowed disabled:opacity-40"><Download size={16} />Export CSV</button>
          {pipelines.length > 0 && (
            <label className="relative">
              ...
              <select
                ...
                className="appearance-none rounded-sm border border-white/10 bg-[#10142E] py-3 pl-4 pr-10 text-xs font-bold text-white outline-none transition focus:border-emerald-400/50"
              >
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-sm border border-white/10 bg-white/5 px-4 py-3 text-xs font-black uppercase tracking-wider text-slate-200 transition hover:border-emerald-400/30 hover:bg-emerald-400/10 disabled:cursor-not-allowed disabled:opacity-40"
          >
          ...
          <button
            ...
            disabled={!selectedPipeline}
            className="flex items-center gap-2 rounded-sm bg-[#059669] px-4 py-3 text-xs font-black uppercase tracking-wider text-white   transition hover:bg-emerald-300 disabled:cursor-not-allowed disabled:opacity-40"
          >
            <Plus size={16} /> Add Lead
          </button>
          {selectedPipeline && <span className={`inline-flex items-center gap-2 rounded-sm border px-3 py-2 text-[10px] font-black uppercase tracking-wider ${followUpCounts.overdue > 0 ? 'border-amber-400/25 bg-amber-400/[0.08] text-amber-300' : 'border-white/10 bg-white/[0.03] text-slate-500'}`}><Calendar size={14} />{followUpCounts.overdue} overdue follow-up{followUpCounts.overdue === 1 ? '' : 's'}</span>}
        </div>
      </div>

      {!activeBranch ? (
        <div className="rounded-sm border border-dashed border-white/15 bg-[#10142E] px-6 py-16 text-center">
        ...
        <div className="flex items-center justify-center gap-3 rounded-sm border border-white/10 bg-[#10142E] py-20 text-sm text-slate-400">
          <Loader2 className="animate-spin text-emerald-300" size={20} /> Loading lead pipelines…
        </div>
      ) : pipelines.length === 0 ? (
        <div className="rounded-sm border border-dashed border-amber-400/25 bg-[#10142E] px-6 py-16 text-center">
        ...
        <>
          <div className="flex flex-col gap-4 rounded-sm border border-white/10 bg-[#10142E] p-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-wrap gap-2">
              {STATUS_FILTERS.map(filter => (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-[10px] font-black uppercase tracking-widest transition ${statusFilter === filter.value ? 'bg-[#059669] text-white' : 'bg-white/5 text-slate-400 hover:bg-white/10 hover:text-white'}`}
                >
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <label className="relative">
                ...
                <select
                  ...
                  className="w-full appearance-none rounded-sm border border-white/10 bg-[#0A0E27] py-2.5 pl-4 pr-10 text-xs font-bold text-slate-200 outline-none focus:border-emerald-400/50 sm:w-48"
                >
              ...
              <label className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-sm border border-white/10 bg-[#0A0E27] py-2.5 pl-10 pr-10 text-xs text-white outline-none placeholder:text-slate-600 focus:border-emerald-400/50 sm:w-72"
                />
                {search && <button type="button" onClick={() => { setSearch(''); setPage(1); }} aria-label="Clear lead search" className="absolute right-2 top-1.5 rounded-sm p-1.5 text-slate-500 hover:bg-white/5 hover:text-white"><X size={14} /></button>}
              </label>
            </div>
          </div>
          ...
          ) : <div className="overflow-hidden rounded-sm border border-white/10 bg-[#10142E]">
            {loadingLeads ? (
              <div className="flex items-center justify-center gap-3 py-24 text-sm text-slate-400">
                <Loader2 className="animate-spin text-emerald-300" size={20} /> Loading leads…
              </div>
              ...
              <div>
                <div className="overflow-x-auto">
                  ...
                  <thead className="border-b border-white/10 bg-white/[0.025] text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">
                    <tr>
                      <th className="px-4 py-4"><input type="checkbox" checked={allVisibleSelected} onChange={toggleAllVisible} aria-label="Select all visible leads" className="h-4 w-4 accent-emerald-400" /></th>
                    ...
                    {paginatedLeads.map(lead => {
                      ...
                      return (
                        <React.Fragment key={lead.id}>
                          <tr onClick={() => toggleLeadDetails(lead)} className="cursor-pointer transition hover:bg-emerald-400/[0.035]">
                            <td className="px-4 py-5" onClick={event => event.stopPropagation()}><input type="checkbox" checked={selectedLeadIds.has(lead.id)} onChange={() => toggleLeadSelection(lead.id)} aria-label={`Select ${profileName(lead)}`} className="h-4 w-4 accent-emerald-400" /></td>
                            <td className="px-6 py-5">
                              <div className="flex items-center gap-3">
                                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-sm bg-emerald-400/10 text-xs font-black text-emerald-300">
                            ...
                            <td className="px-4 py-5" onClick={event => event.stopPropagation()}>
                              <div className="relative w-40">
                                <select
                                  ...
                                  className="w-full appearance-none rounded-sm border border-emerald-400/15 bg-emerald-400/[0.06] py-2 pl-3 pr-8 text-xs font-bold capitalize text-emerald-200 outline-none focus:border-emerald-400/50 disabled:opacity-50"
                                >
                                  {selectedPipeline?.stages.map(stage => <option key={stage} value={stage}>{stage}</option>)}
                                </select>
                                {stageSavingId === lead.id ? <Loader2 className="pointer-events-none absolute right-2.5 top-2.5 h-3.5 w-3.5 animate-spin text-emerald-300" /> : <ChevronDown className="pointer-events-none absolute right-2.5 top-2.5 h-3.5 w-3.5 text-emerald-300" />}
                              </div>
                            </td>
                            ...
                            <td className="px-4 py-5 text-slate-500"><ChevronRight className={`h-4 w-4 transition ${isExpanded ? 'rotate-90 text-emerald-300' : ''}`} /></td>
                          </tr>
                          {isExpanded && (
                            <tr>
                              <td colSpan={9} className="bg-[#0A0E27]/70 px-6 py-6">
                                <div className="grid gap-5 lg:grid-cols-[1.4fr_1fr]">
                                  <div className="space-y-4">
                                    <div className="flex flex-wrap gap-2">
                                      <button type="button" onClick={() => setQuickActivity({ lead, kind: 'call' })} className="rounded-sm border border-emerald-400/20 bg-emerald-400/[0.06] px-3 py-2 text-[10px] font-black uppercase tracking-wider text-emerald-300 hover:bg-emerald-400/10">Log Call</button>
                                      <button type="button" onClick={() => setQuickActivity({ lead, kind: 'note' })} className="rounded-sm border border-white/10 bg-white/[0.04] px-3 py-2 text-[10px] font-black uppercase tracking-wider text-slate-300 hover:bg-white/[0.08]">Log Note</button>
                                      <button type="button" onClick={() => setQuickActivity({ lead, kind: 'meeting' })} className="rounded-sm border border-indigo-400/20 bg-indigo-400/[0.06] px-3 py-2 text-[10px] font-black uppercase tracking-wider text-indigo-300 hover:bg-indigo-400/10">Log Meeting</button>
                                      <button type="button" onClick={() => void openEmailModal(lead)} className="rounded-sm border border-rose-400/20 bg-rose-400/[0.06] px-3 py-2 text-[10px] font-black uppercase tracking-wider text-rose-300 hover:bg-rose-400/10">Send Email</button>
                                      <button type="button" onClick={() => { setQuoteLead(lead); setAcceptedQuoteLogged(false); }} className="rounded-sm border border-amber-400/20 bg-amber-400/[0.06] px-3 py-2 text-[10px] font-black uppercase tracking-wider text-amber-300 hover:bg-amber-400/10">Log Quote</button>
                                    </div>
                                    ...
                                    <label className="block rounded-sm border border-white/10 bg-white/[0.025] p-4">
                                      <span className="mb-2 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-500">
                                        Notes {savingDetail === 'notes' && <Loader2 className="h-3 w-3 animate-spin text-emerald-300" />}
                                      </span>
                                      <textarea
                                        ...
                                        rows={4}
                                        className="w-full resize-none rounded-sm border border-white/10 bg-[#10142E] p-3 text-sm text-slate-200 outline-none placeholder:text-slate-600 focus:border-emerald-400/40"
                                  ...
                                  <div className="space-y-4">
                                    <div className="grid grid-cols-2 gap-3">
                                      ...
                                      <div className="rounded-sm border border-white/10 bg-white/[0.025] p-4">
                                        <UserPlus className="mb-2 text-emerald-300" size={17} />
                                    ...
                                    <label className="block rounded-sm border border-white/10 bg-white/[0.025] p-4">
                                      <span className="mb-2 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-500">
                                        <Calendar size={13} /> Next Action {savingDetail === 'next_action_at' && <Loader2 className="h-3 w-3 animate-spin text-emerald-300" />}
                                      </span>
                                      <input
                                        type="date"
                                        ...
                                        className="w-full rounded-sm border border-white/10 bg-[#10142E] p-3 text-sm text-slate-200 outline-none focus:border-emerald-400/40"
                                      />
                                    </label>
                                    <div className="rounded-sm border border-white/10 bg-white/[0.025] p-4">
                                      ...
                                      <div className="mt-3 flex flex-wrap gap-2">
                                        {tags.length > 0 ? tags.map(tag => <span key={tag} className="inline-flex items-center gap-1 rounded-full bg-emerald-400/10 px-2 py-1 text-[9px] font-black uppercase tracking-wider text-emerald-300"><Tag size={10} />{tag}</span>) : <span className="text-[10px] text-slate-500">Tags are not included in the lead response.</span>}
                                      </div>
                                    </div>
                                  </div>
                                </div>
                ...
                <div className="flex flex-col gap-3 border-t border-white/10 px-5 py-4 text-xs text-slate-400">
                  ...
                  <div className="flex max-w-[350px] flex-wrap items-center gap-3">
                    <label className="flex items-center gap-2">
                      <span>Rows</span>
                      <select value={pageSize} onChange={event => { setPageSize(Number(event.target.value)); setPage(1); }} aria-label="Rows per page" className="rounded-sm border border-white/10 bg-[#0A0E27] px-2 py-1.5 text-xs font-bold text-white outline-none focus:border-emerald-400/50">
                    ...
                    <button type="button" onClick={() => setPage(pagination.page - 1)} disabled={pagination.page === 1} aria-label="Previous leads page" className="flex items-center gap-1 rounded-sm border border-white/10 px-3 py-2 font-bold text-slate-300 hover:border-emerald-400/30 hover:text-emerald-300 disabled:cursor-not-allowed disabled:opacity-30"><ChevronLeft size={15} /> Previous</button>
                    <button type="button" onClick={() => setPage(pagination.page + 1)} disabled={pagination.page === pagination.totalPages} aria-label="Next leads page" className="flex items-center gap-1 rounded-sm border border-white/10 px-3 py-2 font-bold text-slate-300 hover:border-emerald-400/30 hover:text-emerald-300 disabled:cursor-not-allowed disabled:opacity-30">Next <ChevronRight size={15} /></button>
                  </div>
                </div>
              </div>
            )}
        ...
        <CrmModal title="Add Lead" subtitle={`${activeBranch.name} · ${selectedPipeline.name}`} icon={UserPlus} pending={submittingLead} maxWidth="max-w-3xl" onClose={() => setShowAddModal(false)}>
          <form onSubmit={submitLead} className="space-y-5 p-6">
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="space-y-2 text-xs font-bold text-slate-300">First name *<input autoFocus value={leadForm.first_name} onChange={event => setLeadForm(current => ({ ...current, first_name: event.target.value }))} className="w-full rounded-sm border border-white/10 bg-[#0A0E27] p-3 text-white outline-none focus:border-emerald-400/50" /></label>
              <label className="space-y-2 text-xs font-bold text-slate-300">Last name<input value={leadForm.last_name} onChange={event => setLeadForm(current => ({ ...current, last_name: event.target.value }))} className="w-full rounded-sm border border-white/10 bg-[#0A0E27] p-3 text-white outline-none focus:border-emerald-400/50" /></label>
              <label className="space-y-2 text-xs font-bold text-slate-300">Email *<input type="email" value={leadForm.email} onChange={event => setLeadForm(current => ({ ...current, email: event.target.value }))} className={`w-full rounded-sm border bg-[#0A0E27] p-3 text-white outline-none ${leadForm.email && !formEmailValid ? 'border-rose-400/60' : 'border-white/10 focus:border-emerald-400/50'}`} /></label>
              <label className="space-y-2 text-xs font-bold text-slate-300">Phone<input value={leadForm.phone || ''} onChange={event => setLeadForm(current => ({ ...current, phone: event.target.value }))} className="w-full rounded-sm border border-white/10 bg-[#0A0E27] p-3 text-white outline-none focus:border-emerald-400/50" /></label>
              <label className="space-y-2 text-xs font-bold text-slate-300">Source<select value={leadForm.source} onChange={event => setLeadForm(current => ({ ...current, source: event.target.value }))} className="w-full rounded-sm border border-white/10 bg-[#0A0E27] p-3 text-white outline-none focus:border-emerald-400/50"><option value="manual">Manual</option><option value="tower_farm_form">Tower Farm form</option><option value="referral">Referral</option><option value="other">Other</option></select></label>
              <label className="space-y-2 text-xs font-bold text-slate-300">Estimated value<input type="number" min="0" step="0.01" value={leadForm.estimated_value ?? ''} onChange={event => setLeadForm(current => ({ ...current, estimated_value: event.target.value ? Number(event.target.value) : undefined }))} className="w-full rounded-sm border border-white/10 bg-[#0A0E27] p-3 text-white outline-none focus:border-emerald-400/50" /></label>
            </div>
            <label className="block space-y-2 text-xs font-bold text-slate-300">Inquiry<textarea rows={3} value={leadForm.inquiry_text || ''} onChange={event => setLeadForm(current => ({ ...current, inquiry_text: event.target.value }))} className="w-full resize-none rounded-sm border border-white/10 bg-[#0A0E27] p-3 text-white outline-none focus:border-emerald-400/50" /></label>
            <label className="block space-y-2 text-xs font-bold text-slate-300">Notes<textarea rows={3} value={leadForm.notes || ''} onChange={event => setLeadForm(current => ({ ...current, notes: event.target.value }))} className="w-full resize-none rounded-sm border border-white/10 bg-[#0A0E27] p-3 text-white outline-none focus:border-emerald-400/50" /></label>
            <div className="flex justify-end gap-3 border-t border-white/10 pt-5">
              <button type="button" onClick={() => setShowAddModal(false)} disabled={submittingLead} className="rounded-sm px-4 py-3 text-xs font-black uppercase tracking-wider text-slate-400 hover:bg-white/5">Cancel</button>
              <button type="submit" disabled={submittingLead || !leadForm.first_name.trim() || !formEmailValid} className="flex items-center gap-2 rounded-sm bg-[#059669] px-5 py-3 text-xs font-black uppercase tracking-wider text-white disabled:cursor-not-allowed disabled:opacity-40">{submittingLead ? <Loader2 className="animate-spin" size={16} /> : <Plus size={16} />} Create Lead</button>
            </div>
          </form>
        </CrmModal>
      )}
          ...
          <div className="p-6">
            <div className="mb-6 flex items-center gap-3">
              {['Paste', 'Preview', 'Result'].map((label, index) => {
                ...
                return <React.Fragment key={label}><div className={`flex h-8 w-8 items-center justify-center rounded-full text-xs font-black ${complete ? 'bg-emerald-400 text-white' : active ? 'bg-[#059669] text-white' : 'bg-white/5 text-slate-500'}`}>{complete ? <Check size={15} /> : index + 1}</div><span className={`text-[10px] font-black uppercase tracking-widest ${active ? 'text-white' : 'text-slate-500'}`}>{label}</span>{index < 2 && <div className="h-px flex-1 bg-white/10" />}</React.Fragment>;
              })}
            </div>

            {importStep === 'paste' && (
              <div className="space-y-5">
                <div className="rounded-sm border border-emerald-400/15 bg-emerald-400/[0.04] p-4 text-xs leading-5 text-slate-300">
                  <Upload className="mb-2 text-emerald-300" size={18} />
                  Paste tab- or comma-separated rows, or upload a spreadsheet. Email, name, and phone columns are detected from their contents.
                </div>
                <label
                  className={`flex cursor-pointer flex-col items-center justify-center rounded-sm border border-dashed px-6 py-8 text-center transition ${draggingFile ? 'border-emerald-300 bg-emerald-400/10' : 'border-white/15 bg-[#0A0E27] hover:border-emerald-400/40 hover:bg-emerald-400/[0.03]'}`}
                ...
                >
                  {readingFile ? <Loader2 className="mb-3 animate-spin text-emerald-300" size={25} /> : <FileSpreadsheet className="mb-3 text-emerald-300" size={25} />}
                ...
                <textarea autoFocus rows={9} value={pasteRaw} onChange={event => setPasteRaw(event.target.value)} placeholder="Paste rows from your spreadsheet (tab or comma separated)…" className="w-full resize-y rounded-sm border border-white/10 bg-[#0A0E27] p-4 font-mono text-xs leading-6 text-slate-200 outline-none placeholder:text-slate-600 focus:border-emerald-400/50" />
                <div className="flex justify-end"><button type="button" onClick={previewImport} disabled={!pasteRaw.trim() || readingFile} className="flex items-center gap-2 rounded-sm bg-[#059669] px-5 py-3 text-xs font-black uppercase tracking-wider text-white disabled:opacity-40">Preview <ChevronRight size={16} /></button></div>
              </div>
            )}
              ...
              <div className="space-y-5">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <p className="flex items-center gap-2 text-sm font-black text-white">
                    {checkingExisting && <Loader2 className="animate-spin text-emerald-300" size={15} />}
                    {importableRows.length} ready, {openLeadRows.length} already in pipeline, {parsedSkipped.length} skipped
                  </p>
                  <button type="button" onClick={() => setImportStep('paste')} className="text-xs font-bold text-emerald-300 hover:text-emerald-200">Choose different input</button>
                </div>
                {sheetInfo && <div className="rounded-sm border border-emerald-400/15 bg-emerald-400/[0.04] px-4 py-3 text-xs text-emerald-200">{sheetInfo}</div>}
                <label className="flex max-w-xs flex-col gap-2 text-xs font-bold text-slate-300">
                  Import source
                  <select value={importSource} onChange={event => setImportSource(event.target.value)} className="rounded-sm border border-white/10 bg-[#0A0E27] p-3 text-white outline-none focus:border-emerald-400/50">
                ...
                <div className="max-h-80 overflow-auto rounded-sm border border-white/10">
                  <table className="w-full min-w-[620px] text-left text-xs">
                    ...
                    <tbody className="divide-y divide-white/[0.06]">
                      {importableRows.map((row, index) => {
                        ...
                        return <tr key={`${row.email}-${index}`}><td className="px-4 py-3">{known ? <span className="inline-flex rounded-full border border-emerald-400/25 bg-emerald-400/10 px-2 py-1 text-[9px] font-black uppercase tracking-wider text-emerald-300">Known contact</span> : <span className="inline-flex items-center gap-1 text-emerald-300"><Check size={13} /> Ready</span>}</td><td className="px-4 py-3 text-slate-200">{[row.first_name, row.last_name].filter(Boolean).join(' ') || '—'}</td><td className="px-4 py-3 text-slate-300">{row.email}</td><td className="px-4 py-3 text-slate-400">{row.phone || '—'}</td><td className="max-w-64 truncate px-4 py-3 text-slate-500" title={row.notes}>{row.notes || '—'}</td></tr>;
                      })}
                    </tbody>
                  </table>
                </div>
                {parsedRows.length === 0 && <div className="rounded-sm border border-dashed border-white/10 py-10 text-center text-sm text-slate-500">No valid email rows were found.</div>}
                {openLeadRows.length > 0 && <div className="rounded-sm border border-amber-400/20 bg-amber-400/[0.04]"><div className="border-b border-amber-400/15 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-amber-300">Already open leads · excluded</div><div className="divide-y divide-white/[0.06]">{openLeadRows.map(row => <div key={row.email} className="flex items-center justify-between gap-4 px-4 py-3 text-xs"><span className="text-slate-300">{row.email}</span><span className="rounded-full bg-amber-400/10 px-2 py-1 text-[9px] font-black uppercase tracking-wider text-amber-300">Already open lead</span></div>)}</div></div>}
                {parsedSkipped.length > 0 && <div className="rounded-sm border border-white/10"><div className="border-b border-white/10 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-500">Skipped while parsing</div><div className="max-h-44 divide-y divide-white/[0.06] overflow-auto">{parsedSkipped.map((skip, index) => <div key={`${skip.line}-${index}`} className="flex items-start justify-between gap-4 px-4 py-3 text-xs"><span className="min-w-0 break-all text-slate-400">{skip.line || 'Empty row'}</span><span className="shrink-0 text-right text-rose-300">{skip.reason}</span></div>)}</div></div>}
                <div className="flex justify-end"><button type="button" onClick={confirmImport} disabled={importing || checkingExisting || importableRows.length === 0} className="flex items-center gap-2 rounded-sm bg-[#059669] px-5 py-3 text-xs font-black uppercase tracking-wider text-white disabled:opacity-40">{importing ? <Loader2 className="animate-spin" size={16} /> : <Upload size={16} />} Import {importableRows.length} Lead{importableRows.length === 1 ? '' : 's'}</button></div>
              </div>
            )}

            {importStep === 'result' && importSummary && (
              <div className="space-y-5">
                <div className="rounded-sm border border-emerald-400/20 bg-emerald-400/[0.06] p-6 text-center"><div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-emerald-400 text-white"><Check size={22} /></div><h3 className="text-xl font-black text-white">{importSummary.created} lead{importSummary.created === 1 ? '' : 's'} created</h3><p className="mt-1 text-sm text-slate-400">{importSummary.skipped.length} row{importSummary.skipped.length === 1 ? '' : 's'} skipped</p></div>
                {importSummary.skipped.length > 0 && <div className="rounded-sm border border-white/10"><div className="border-b border-white/10 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-500">Skipped rows</div><div className="divide-y divide-white/[0.06]">{importSummary.skipped.map((skip, index) => <div key={`${skip.line}-${index}`} className="flex items-start justify-between gap-4 px-4 py-3 text-xs"><span className="min-w-0 break-all text-slate-300">{skip.line || 'Missing row'}</span><span className="shrink-0 text-right text-rose-300">{skip.reason}</span></div>)}</div></div>}
                <div className="flex justify-end"><button type="button" onClick={closeImport} className="rounded-sm bg-[#059669] px-5 py-3 text-xs font-black uppercase tracking-wider text-white">Done</button></div>
              </div>
            )}
          </div>
        </CrmModal>
