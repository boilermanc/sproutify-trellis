...
import { createPlannedStudioTrack, createStudioAlbum, deletePlannedStudioTrack, generateStudioTrack, getStudioAlbums, getStudioTracks, planStudioAlbum, planStudioTrack, reviewStudioTrack, StudioTrackDraft, updatePlannedStudioTrack } from '../services/studioAlbumsService';
...
const StudioAlbums: React.FC<Props> = ({ addToast }) => {
  ...
  const [batchTrackCount, setBatchTrackCount] = useState(8);
  ...
  useEffect(() => { if (selected) setBatchTrackCount(suggestedTrackCount); }, [selected?.id, tracks.length]);
  ...
  const planAlbum = async () => {
    if (!selected) return;
    setPlanningAlbum(true);
    try { const planned = await planStudioAlbum(selected.id, batchTrackCount); setTracks(current => [...current, ...planned]); addToast(`${planned.length} tracks added to the album plan.`, 'success'); }
  ...
  return <div className="max-w-7xl mx-auto space-y-8">
    ...
    {selected && <section className="bg-white border border-slate-200 rounded-[2rem] p-6 lg:p-8"><div className="flex justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-widest text-emerald-600">AI Music Director</p><h2 className="text-xl font-black text-slate-800">{selected.title}</h2><p className="text-sm text-slate-500">Build the tracklist first. Planned tracks remain editable until audio generation begins.</p></div><button onClick={() => setSelected(null)} className="text-slate-400 hover:text-slate-800"><X /></button></div>
      ...
      <div className="mt-4 flex flex-wrap gap-3 items-end bg-emerald-50 border border-emerald-100 rounded-2xl p-4"><label className="text-xs font-bold text-slate-600">AI tracks to add<input type="number" min="1" max="24" value={batchTrackCount} onChange={e => setBatchTrackCount(Math.max(1, Math.min(24, Number(e.target.value) || 1)))} className="mt-1.5 block w-28 bg-white border border-emerald-200 rounded-xl px-3 py-2 text-sm" /></label><button type="button" onClick={planAlbum} disabled={planningAlbum} className="inline-flex items-center gap-2 bg-emerald-600 text-white px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider disabled:opacity-50">{planningAlbum ? <Loader2 size={16} className="animate-spin" /> : <Wand2 size={16} />}{tracks.length ? `Add ${batchTrackCount} planned track${batchTrackCount === 1 ? '' : 's'}` : 'Plan full album'}</button><p className="text-xs text-emerald-800">{durationDelta < 0 ? `${suggestedTrackCount} track${suggestedTrackCount === 1 ? '' : 's'} will fill the remaining time at up to 165 seconds each.` : 'Add more tracks whenever you want a longer release.'} No music credits are used.</p></div>
