...
import { createStudioAlbum, generateStudioTrack, getStudioAlbums, getStudioTracks, planStudioAlbum, planStudioTrack, reviewStudioTrack } from '../services/studioAlbumsService';
...
const StudioAlbums: React.FC<Props> = ({ addToast }) => {
  ...
  const planAlbum = async () => {
    ...
    try {
      const planned = await planStudioAlbum(selected.id, batchTrackCount);
      setTracks(planned);
      addToast(`${planned.length} tracks planned. Review the set before generating any audio.`, 'success');
