...
const StudioAlbums: React.FC<Props> = ({ addToast }) => {
  ...
  const planAlbum = async () => {
    ...
    try { const planned = await planStudioAlbum(selected.id, batchTrackCount); setTracks(planned); addToast(`${planned.length} tracks planned. Adjust the tracklist before generation.`, 'success'); }
  ...
  return <div className="max-w-7xl mx-auto space-y-8">
    ...
    {selected && <section className="bg-white border border-slate-200 rounded-[2rem] p-6 lg:p-8"><div className="flex justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-widest text-emerald-600">AI Music Director</p><h2 className="text-xl font-black text-slate-800">{selected.title}</h2><p className="text-sm text-slate-500">Build the tracklist first. Planned tracks remain editable until audio generation begins.</p></div><button onClick={() => setSelected(null)} className="text-slate-400 hover:text-slate-800"><X /></button></div>
      ...
      {tracks.length === 0 && <div className="mt-6 flex flex-wrap gap-3 items-end bg-emerald-50 border border-emerald-100 rounded-2xl p-4"><label className="text-xs font-bold text-slate-600">Tracks to plan<input type="number" min="2" max="24" value={batchTrackCount} onChange={e => setBatchTrackCount(Number(e.target.value))} className="mt-1.5 block w-28 bg-white border border-emerald-200 rounded-xl px-3 py-2 text-sm" /></label><button type="button" onClick={planAlbum} disabled={planningAlbum} className="inline-flex items-center gap-2 bg-emerald-600 text-white px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider disabled:opacity-50">{planningAlbum ? <Loader2 size={16} className="animate-spin" /> : <Wand2 size={16} />} Plan full album</button><p className="text-xs text-emerald-800">No music credits are used until you choose Generate.</p></div>}
