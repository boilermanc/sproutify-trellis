...
import { Album, Check, Loader2, Music2, Plus, Sparkles, X } from 'lucide-react';
import { StudioAlbum, StudioTrack } from '../types';
import { createStudioAlbum, generateStudioTrack, getStudioAlbums, getStudioTracks, reviewStudioTrack } from '../services/studioAlbumsService';
...
const StudioAlbums: React.FC<Props> = ({ addToast }) => {
  ...
  return <div className="max-w-7xl mx-auto space-y-8">
    ...
    <section className="grid lg:grid-cols-[1.15fr_.85fr] gap-8 items-start"><form onSubmit={submit} className="bg-white border border-slate-200 rounded-[2rem] p-6 lg:p-8 space-y-5"><div><p className="text-[10px] font-black uppercase tracking-widest text-emerald-600">New production</p><h2 className="text-xl font-black text-slate-800">Start an album brief</h2></div><div className="grid sm:grid-cols-2 gap-4">{[['title','Album title'],['artist_name','Fictional artist name'],['genre','Genre'],['mood','Mood'],['era','Era'],['theme','Theme or setting']].map(([field,label]) => <label key={field} className="text-xs font-bold text-slate-600">{label}<input value={form[field as keyof typeof form] as string} onChange={e => update(field as keyof typeof EMPTY, e.target.value)} className="mt-1.5 w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm" /></label>)}</div><label className="text-xs font-bold text-slate-600 block">Session description<textarea value={form.description} onChange={e => update('description', e.target.value)} className="mt-1.5 w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm min-h-20" /></label><div className="grid sm:grid-cols-2 gap-4"><label className="text-xs font-bold text-slate-600">Target duration (minutes)<input type="number" min="1" max="240" value={form.targetMinutes} onChange={e => update('targetMinutes', Number(e.target.value))} className="mt-1.5 w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm" /></label><label className="text-xs font-bold text-slate-600">Vocals<select value={form.vocal_direction} onChange={e => update('vocal_direction', e.target.value)} className="mt-1.5 w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm"><option value="instrumental">Instrumental</option><option value="mostly_instrumental">Mostly instrumental</option><option value="vocals">Vocals</option></select></label></div><button disabled={creating} className="inline-flex items-center gap-2 bg-emerald-600 text-white px-5 py-3 rounded-xl text-xs font-black uppercase tracking-wider disabled:opacity-50">{creating ? <Loader2 size={16} className="animate-spin" /> : <Plus size={16} />} Create album</button></form>
    ...
    {selected && <section className="bg-white border border-slate-200 rounded-[2rem] p-6 lg:p-8"><div className="flex justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-widest text-emerald-600">Single-track adapter</p><h2 className="text-xl font-black text-slate-800">{selected.title}</h2><p className="text-sm text-slate-500">Generate one track through the proven legacy Lyria queue, then register it as a private Studio asset.</p></div><button onClick={() => setSelected(null)} className="text-slate-400 hover:text-slate-800"><X /></button></div><form onSubmit={generateTrack} className="mt-6 grid lg:grid-cols-[1fr_2fr_120px_auto] gap-3 items-end"><label className="text-xs font-bold text-slate-600">Track title<input value={trackForm.title} onChange={e => setTrackForm(current => ({ ...current, title: e.target.value }))} className="mt-1.5 w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm" /></label><label className="text-xs font-bold text-slate-600">Generation prompt<input value={trackForm.prompt} onChange={e => setTrackForm(current => ({ ...current, prompt: e.target.value }))} placeholder="Original instrumental jazz, muted trumpet…" className="mt-1.5 w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm" /></label><label className="text-xs font-bold text-slate-600">Seconds<input type="number" min="15" max="165" value={trackForm.duration_seconds} onChange={e => setTrackForm(current => ({ ...current, duration_seconds: Number(e.target.value) }))} className="mt-1.5 w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm" /></label><button disabled={trackBusy} className="bg-slate-950 text-white px-4 py-3 rounded-xl text-xs font-black uppercase tracking-wider disabled:opacity-50">{trackBusy ? <Loader2 className="animate-spin" size={16} /> : 'Generate'}</button></form><div className="mt-6 space-y-3">{tracks.length === 0 ? <p className="text-sm text-slate-500">No Studio tracks yet.</p> : tracks.map(track => <div key={track.id} className="border border-slate-200 rounded-2xl p-4 flex flex-wrap gap-4 items-center justify-between"><div><p className="font-black text-slate-800">{track.track_number}. {track.title}</p><p className="text-xs text-slate-500">{track.duration_seconds || '—'} sec · {track.review_status.replaceAll('_', ' ')}</p></div><div className="flex gap-2 items-center">{track.audio_url && <audio controls src={track.audio_url} className="h-8 max-w-[220px]" />} {track.review_status === 'pending_review' && <><button onClick={() => reviewTrack(track, true)} className="p-2 text-emerald-600 bg-emerald-50 rounded-lg" title="Approve"><Check size={16} /></button><button onClick={() => reviewTrack(track, false)} className="p-2 text-rose-600 bg-rose-50 rounded-lg" title="Reject"><X size={16} /></button></>} {track.review_status === 'regenerating' && <Loader2 className="animate-spin text-amber-600" size={18} />} {track.review_status === 'failed' && <span className="text-xs text-rose-600">{track.rejection_reason}</span>}</div></div>)}</div></section>}
  </div>;
};
